...
def biased_walker() -> BiasedWalker:
    return BiasedWalker("Test Walker", is_3d=True, mass=1.0, bias="Origin", bias_scale=2)
...
def test_generate_move_angle_2d() -> None:
    biased_walker = BiasedWalker("Test Walker", is_3d=False, mass=1.0, bias="Left", bias_scale=2)
    move_angle = biased_walker._generate_move_angle()
    
    assert isinstance(move_angle, tuple)
    
def test_biased_walker_not_in_dict() -> None:
    walker = BiasedWalker("Test Walker", is_3d=True, mass=1.0, bias="invalid", bias_scale=2)
