...
class WalkerFrame(ctk.CTkFrame):  # type: ignore[misc]
    ...
    __WALKER_TYPES = ["Straight", "Random Angle", "Random", "Biased", "Accelerating", "Stocks", "Resetable"]
    ...
    def __init__(self, master: ctk.CTkFrame) -> None:
        ...
        self.acceleration_entry_text = ctk.CTkLabel(
            self.acceleration_entry_frame, 0, text="Acceleration type:"
        )
        ...
        # layout
        self.walker_choose_dropdown.pack(
            expand=True, side="left", padx=self.padding, pady=self.padding
        )
        ...
        self.delete_button.pack(
            expand=True, side="right", padx=self.padding, pady=self.padding
        )
    ...
    def get_walker(self, name: str) -> Walker:
        ...
        if self.walker_type.get() == "Random Angle":
            return RandomAngleWalker(
                name, self.dimension_toggle.get(), self.mass_spinbox.get()
            )
        if self.walker_type.get() == "Random":
            return RandomWalker(
                name, self.dimension_toggle.get(), self.mass_spinbox.get()
            )
        if self.walker_type.get() == "Resetable":
            return ResetableWalker(
                name, self.dimension_toggle.get(), self.mass_spinbox.get()
            )
    ...
    def pack_walker_specific_widgets(self, current_value: str) -> None:
        ...
        if current_value == "Biased":
            ...
            self.bias_entry_frame.pack(
                side="left", padx=self.padding, pady=self.padding
            )
            ...
            self.acceleration_entry_frame.pack(
                side="left", padx=self.padding, pady=self.padding
            )
